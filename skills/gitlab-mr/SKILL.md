...
- Keep everything **concise and short** — brevity is required, not optional:
  - `Summary`: 1-2 sentences max, under 30 words. No filler, no background the reviewer already knows. **If this MR is a fix for a previous merged MR (post-merge / post-QA fix), you MUST mention the previous MR in the Summary.** Add a short reference at the end of Summary, e.g. `Follow-up to https://gitlab.com/group/project/-/merge_requests/123 — fixes ...` or `Fix for https://gitlab.com/group/project/-/merge_requests/123 — addresses QA feedback on ...`. Get the previous MR URL from `glab mr list --merged`, `git log --grep`, or ask the user for the old MR link if not discoverable. Keep it on the same line or a second short sentence. Do NOT include the `!123` shorthand alongside the URL — GitLab auto-links `!123` and renders two links for the same MR; use the raw URL only.
